...
from utils.loss_utils import l1_loss, ssim, EdgePreservingSmoothnessLoss, SmoothnessLoss, l2_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = GaussianModel(dataset.sh_degree, dataset.ref_sh_degree, opt.optimizer_type)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        ref_map_smooth_loss = smooth_loss_func(ref_map.view(-1, H, W))
        ref_map_smooth_loss *= 0.001
        # print(f"Iteration {iteration}: ref map smooth loss: {ref_map_smooth_loss.item():.4f}")
        loss += ref_map_smooth_loss
        ...
        Ll1depth_pure = 0.0
        # print(depth_l1_weight(iteration))
        # print(viewpoint_cam.depzth_reliable)
        if depth_l1_weight(iteration) > 0 and viewpoint_cam.depth_reliable:
            invDepth = render_pkg["depth"]
            # print(invDepth)
            mono_invdepth = viewpoint_cam.invdepthmap.cuda()

            # min_depth1 = torch.min(mono_invdepth).item()
            # min_depth2 = torch.min(invDepth).item()

            # max_depth1 = torch.max(mono_invdepth).item()
            # max_depth2 = torch.max(invDepth).item()

            # print(f"Min depth: {min_depth1}")
            # print(f"Min depth: {min_depth2}")
            # print(f"Max depth: {max_depth1}")
            
            

            
            # print(f"Max depth: {max_depth2}")
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, init_loss, loss, l1_loss, ref_map_smooth_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background, 1., SPARSE_ADAM_AVAILABLE, None, dataset.train_test_exp), dataset.train_test_exp)
...
def training_report(tb_writer, iteration, Ll1, init_loss, loss, l1_loss, ref_map_smooth_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, train_test_exp):
    if tb_writer:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[2000, 5000, 7_000, 10000, 15000, 20000, 25000, 30_000])
